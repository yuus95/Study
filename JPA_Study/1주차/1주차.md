...
- 자바 진영의 ORM 기술 표준
  ...
  - 객체와 관게형 데이터베이스르를 중간에서 매핑해주는 역할
- 동작 방식
...
- 사용하는 이유  
  ...
  - __패러다임의 불일치 해결__
    - JPA와 상속 
      ...
      - ```java
        ...
        ```
  - 성능 
    - 튜닝만 잘해준다면 쿼리문 작성보다 보다좋은 성능을 보여준다.
  ...
  - JPA의 성능 최적화 기능
    ...
    - 연관 관계 설정
      ...
      - 즉시 로딩 : JOIN SLQ로 한번ㅂ에 연관된 객체까지 미리 조회 --> 실제로 사용하면 성능이 매우 안좋아진다.
...
- 데이터베이스 방언
  ...
  - persistence.xml 에서 hibernate.dialect속성에서 원하는 데이터베이스를 지정하면 데이터베이스 방언을 지원해준다(40가지 이상의 방언을 지원)
